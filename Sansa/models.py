...
class Asset(models.Model):
	...
	#DateField使用Python的datetime.date实例表示的日期
	...
	#status = models.ForeignKey('Status', verbose_name=u'设备状态',)

	memo = models.TextField(u'备注', null=True, blank=True)
	create_date = models.DateTimeField(blank=True, auto_now_add=True)
	update_date = models.DateTimeField(blank=True, auto_now_add=True)
...
class NetworkDevice(models.Model):
	...
	frimware = models.ForeignKey('software', blank=True, null=True)
	#SmallIntegerField模型字段类似 PositiveIntegerField（正整数字段0到2147483647的值）, 但是只允许小于某一特定值（依据数据库类型而定）。从0 到 32767 这个区间，
...
class Software(models.Model):
	...
	os_type = models.CharField(u'软件类型', choices=os_distribution_choices, max_length=32)
	distribution = models.CharField(u'发型版本', choices=os_types_choice, max_length=32)
	version = models.CharField(u'软件/系统版本', max_length=64, blank=True, null=True)
	language_choices = (
		('cn', u'中文'),
		('en', u'英文')
		)
	language = models.CharField(u'系统语言', choices = language_choices, max_length=32)
...
class EventLog(models.Model):
	...
	user = models.ForeignKey('UserProfile', verbose_name='事件用户')
	...
	def colored_enent_type(self):
		if self.event_type == 1:
			cell_html = '<span style="background:orange;">%s </span>'  
		elif self.event_type == 2:
			cell_html = '<span style="background:yellowgre">>%s </span>'
		else:
			cell_html = '<span> %s</span>'
		return cell_html % self.get_event_type_display()
	colored_enent_type.allow_tags = True
	colored_enent_type.short_description = u'事件类型'



class CPU(models.Model):
	asset = models.OneToOneField('Asset')
	cpu_model = models.CharField(u'CPU类型', max_length=128, blank=True)
...
class RAM(models.Model):
	...
	capcity = models.IntegerField(u'内存大小（MB)')
	memo = models.CharField(u'备注', max_length=128, blank=True, null=True)
	create_date = models.DateTimeField(auto_now_add=True)
	update_date = models.DateTimeField(blank=True, null=True)

	def __str__(self):
		return '%s:%s:%s' % (self.asset_id, self.slot,self.capcity)

	class Meta:
		verbose_name = 'RAM'
		verbose_name_plural = "RAM"
		unique_together = ("asset", "slot")
	auto_create_fields = ['sn','slot','model','capcity']
...
class Disk(models.Model):
	...
	capactity = models.FloatField(u'磁盘容量')
	Disk_iface_choice = (
		('STAT','STAT'),
		('SAS','SAS'),
		('SCSI','SCSI'),
	...
	auto_create_fields = ['sn', 'slot', 'manufactory','models','create_date']
	...
	def  __str__(self):
		return '%s:slot:%s capactity:%s' % (self.asset_id, self.capactity)
...
class NIC(models.Model):
	...
	macaddress = models.CharField(u'MAC', max_length=64, unique=True, blank=True, null=True)
	...
	auto_create_fields = ['name','sn','model','macaddress','ipaddress','memo','create_date','update_date']
